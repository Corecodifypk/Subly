...
public class UnityAdsPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private boolean initialize(Map<?, ?> args) {
        Object rawGameId = args.get(UnityAdsConstants.GAME_ID_PARAMETER);
        String gameId = rawGameId == null ? "" : rawGameId.toString().trim();
        Log.i(TAG, "UnityAdsPlugin.initialize gameId from Flutter: " + gameId);

        if (gameId.isEmpty()) {
            Log.e(TAG, "Unity Ads init aborted: missing gameId argument");
            return false;
        }

        if (UnityAds.isInitialized()) {
            Log.i(TAG, "Unity Ads already initialized — skipping duplicate init");
            return true;
        }
